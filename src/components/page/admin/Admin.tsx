...
export type LoginFunc = (cred:{[x in CredentialsEnum]:string})=>Promise<void|boolean>;


export default class AdminPage extends React.Component<{login: {loggedIn: boolean, email?:string, error?: string, login:LoginFunc, signup:LoginFunc}, isAdmin: boolean, isEditor: boolean, usersManager:UsersManager}, {awaitingResponse?:boolean,redirectToVerifyPage?:boolean}> {
    ...
    onLogin = (isSignup:boolean,...args:Parameters<LoginFunc>):void => {
        ...
        p.then((created)=>this.setState(isSignup&&created!==true?{redirectToVerifyPage:true}:{awaitingResponse:false}));
    };
    
    render():ReactNode {
        const { login: { loggedIn, error, email }, isAdmin, isEditor, usersManager } = this.props;

        if (this.state.redirectToVerifyPage)
            return <Redirect to="/verify-link"/>;
